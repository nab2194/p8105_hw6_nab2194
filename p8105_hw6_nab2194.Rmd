...
glm(bwt ~ smoken + mrace + gaweeks + momage + fincome + smoken * ppwt, data = birthweight_df) %>%
  broom::tidy() %>% 
  knitr::kable()
...
```{r}
glm(bwt ~ smoken + mrace + gaweeks + momage + fincome + ppwt, data = birthweight_df) %>%
  broom::tidy() %>% 
  knitr::kable()
```
...
```{r}
glm(bwt ~ smoken + mrace + gaweeks + momage + ppwt, data = birthweight_df) %>%
  broom::tidy() %>% 
  knitr::kable()
```
...
```{r}
bwmodel = glm(bwt ~ smoken + mrace + gaweeks + ppwt, data = birthweight_df)
...
cv_df = 
  cv_df %>% 
  mutate(
    bwmodel = map(train, ~mgcv::gam(bwt ~ s(smoken + mrace + gaweeks + ppwt), data = birthweight_df)),
...
```{r getting values for a single model}
