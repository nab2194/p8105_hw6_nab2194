...
```
Looks like a linear model will 10000% not work here, based on my above results. Good to know. 
...
lm(bwt ~ smoken + mrace + gaweeks + momage + fincome + smoken * ppwt, data = birthweight_df) %>%
  broom::tidy() %>% 
  knitr::kable()
...
```{r}
lm(bwt ~ smoken + mrace + gaweeks + momage + fincome + ppwt, data = birthweight_df) %>%
  broom::tidy() %>% 
  knitr::kable()
```
...
```{r}
lm(bwt ~ smoken + mrace + gaweeks + momage + ppwt, data = birthweight_df) %>%
  broom::tidy() %>% 
  knitr::kable()
```
...
```{r}
bwmodel = lm(bwt ~ smoken + mrace + gaweeks + ppwt, data = birthweight_df)
...
Model 3 (from HW): 
```{r}
model3 = lm(bwt ~ bhead * blength * babysex, data = birthweight_df)
```


Now I'm going to compare my model with the 2 provided models using cross-validation. 

```{r cross-validation for prob 2 }
...
cv_df = 
  cv_df %>% 
  mutate(
    bwmodel = map(train, ~lm(bwt ~ smoken + mrace + gaweeks + ppwt, data = birthweight_df)),
    model2 = map(train, ~lm(bwt ~ blength + gaweeks, data = birthweight_df)),
    model3 = map(train, ~lm(bwt ~ bhead * blength * babysex, data = birthweight_df))) %>% 
  mutate(
...
## Clearly mine is performing the worst (lol -  sad!)
